...
public:
    ...
    void integrate_dv(Vector7d& q, Vector7d& v, const Vector7d& dv, double dt)
    {
        Vector7d v_mean = v + 0.5*dt*dv;
        v += dt*dv;
        q = pin::integrate(tsid_robot_->model(), q, dt*v_mean);
    }
